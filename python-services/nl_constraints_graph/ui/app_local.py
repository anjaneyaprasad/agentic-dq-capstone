...
def run_graph(dataset: str, prompt: str, apply: bool, feedback: str | None, self_healing: bool):
    """Invoke the LangGraph NL → Rules workflow and return a GraphState."""
...
def load_latest_verification_run(dataset: str) -> tuple[pd.Timestamp, str, pd.DataFrame]:
    ...
    try:
        ...
        # Convert epoch-ms style STARTED_AT / FINISHED_AT if needed
        runs_df = _convert_epoch_ms_columns(runs_df, ["STARTED_AT", "FINISHED_AT"])
        ...
        # Normalize any epoch-ms columns if they exist
        df = _convert_epoch_ms_columns(df, ["STARTED_AT", "FINISHED_AT"])
...
def load_bad_rows_for_run(dataset: str, run_id: str) -> pd.DataFrame:
    ...
    try:
        ...
        bad_df = pd.read_sql(sql, conn, params=[ds, run_id])
        bad_df = _convert_epoch_ms_columns(bad_df, ["CREATED_AT"])
        return bad_df
    finally:
        conn.close()
...
def run_html_report(dataset: str) -> str:
    ...
    logs.append(
        run_cmd(
            [sys.executable, "build_html_report.py", "--dataset", dataset],
...
def main():
    ...
    st.title("🧠 Data Quality NL → Rules Agent Console")
    ...
    apply_changes = st.sidebar.checkbox(
        "Apply changes to Snowflake DQ_RULES (not just dry-run)",
        value=False,
    )
    ...
    st.sidebar.markdown(
        ...
        "Turn it on only when you're happy with the rules to save into Snowflake DQ_RULES."
    )
    ...
    # ----------------------------------------------------
    # TAB 1: NL → Rules Agent (prompt, results, refinement)
    # ----------------------------------------------------
    with tab_agent:
        ...
        with col_right:
            st.subheader("3. Latest Run – Inferred Rules")
            ...
            if run_button:
                if not prompt.strip():
                    ...
                    st.markdown("#### Original Rules (from previous state / Snowflake)")
                    ...
                    # --- Rules payload preview (JSON) ---
                    st.markdown("#### Rules payload preview (JSON)")
                    if final_state.inferred_rules:
                        rules_payload = [r.dict() for r in final_state.inferred_rules]
                        st.code(json.dumps(rules_payload, indent=2), language="json")
                    else:
                        st.write("No rules generated (validation failed or no rules).")

                    # --- Save to Snowflake DQ_RULES if apply_changes is ON ---
                    if apply_changes:
                        if final_state.inferred_rules:
                            rules_to_save = [r.dict() for r in final_state.inferred_rules]
                            try:
                                save_rules_to_snowflake(
                                    dataset.upper(),
                                    rules_to_save,
                                    created_by="NL_AGENT_STREAMLIT",
                                )
                                st.success(
                                    f"Saved {len(rules_to_save)} rules to DQ_RULES for dataset '{dataset.upper()}'."
                                )
                            except Exception as e:
                                st.error(f"Failed to save rules to Snowflake: {e}")
                        else:
                            st.warning(
                                "Apply was requested, but no inferred rules are available to save."
                            )
                    else:
                        st.info(
                            "Dry-run only: rules were not saved to Snowflake. "
                            "Turn on 'Apply changes to Snowflake DQ_RULES' to persist."
                        )
            else:
            ...
            if not st.session_state.last_prompt or not st.session_state.last_dataset:
                ...
                with st.spinner("Running refinement with feedback..."):
                    try:
                        ...
                        st.markdown("#### Refined rules payload preview (JSON)")
                        if refined_state.inferred_rules:
                            refined_payload = [r.dict() for r in refined_state.inferred_rules]
                            st.code(json.dumps(refined_payload, indent=2), language="json")
                        else:
                            st.write("No rules generated (refinement validation failed).")

                        if apply_changes:
                            if refined_state.inferred_rules:
                                rules_to_save = [r.dict() for r in refined_state.inferred_rules]
                                try:
                                    save_rules_to_snowflake(
                                        st.session_state.last_dataset.upper(),
                                        rules_to_save,
                                        created_by="NL_AGENT_REFINEMENT_STREAMLIT",
                                    )
                                    st.success(
                                        f"Saved {len(rules_to_save)} refined rules to DQ_RULES for dataset "
                                        f"'{st.session_state.last_dataset.upper()}'."
                                    )
                                except Exception as e:
                                    st.error(f"Failed to save refined rules to Snowflake: {e}")
                            else:
                                st.warning(
                                    "Apply was requested, but no refined rules are available to save."
                                )
                        else:
                            st.info(
                                "Refinement was dry-run only; rules not saved to Snowflake. "
                                "Enable 'Apply changes' to persist."
                            )
    ...
    with tab_dq_brain:
        ...
        if st.button("Load profiling & generate DQ rule suggestions", key="dq_brain_btn"):
            try:
                with st.spinner(f"Loading latest profiling metrics for {dq_dataset} from Snowflake..."):
                    ...
                    prof_df = _convert_epoch_ms_columns(prof_df, ["RUN_TS"])

                if prof_df.empty:
                    st.warning(
                        f"No profiling metrics found for dataset '{dq_dataset}'. "
                        "Run ProfilingJob first."
                    )
                else:
                    ...
                    try:
                        ...
                        if not llm_json_text.strip():
                            st.info(
                                "LLM JSON is empty. Provide valid JSON above to see rule suggestions."
                            )
                        else:
                            try:
                        ...
                        if rules:
                            # Store in session so Save button works on next reruns
                            st.session_state["dq_brain_rules"] = rules
                            st.success(
        ...
        # 2) Show editor + Save button if we have rules in session
        if "dq_brain_rules" in st.session_state and st.session_state["dq_brain_rules"]:
            rules_df = pd.DataFrame(st.session_state["dq_brain_rules"])
            ...
            if st.button("💾 Save APPROVED rules to DQ_RULES", key="dq_brain_save_btn"):
                ...
                if approved_df.empty:
                    ...
                    rules_to_save = approved_df.drop(columns=["APPROVE"]).to_dict(
                        orient="records"
                    )
                    try:
                        save_rules_to_snowflake(
    ...
    with tab_pipeline:
        ...
        if run_spark_btn:
            with st.spinner("Running Spark DQ jobs via sbt (local Python)..."):
                try:
                    logs = run_spark_jobs(dataset)
                    st.session_state.exec_logs = logs
                    st.success("Spark jobs completed (local).")
                except Exception as e:
    ...
    # ----------------------------------------------------
    with tab_graph:
        st.subheader("8. NL → Rules Agent Graph")
