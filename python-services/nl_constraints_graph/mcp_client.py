...
# def run_dq_tool(dataset: str):
#     """
#     Call dq_latest_profiling over MCP and return a plain Python dict:
...
#                 # Optional: keep tools listing during dev
#                 tools = await session.list_tools()
#                 print(f"[mcp_client] tools detected: {tools}", file=sys.stderr, flush=True)

#                 print(
#                     f"[mcp_client] calling dq_latest_profiling for dataset={dataset}",
#                     file=sys.stderr,
#                     flush=True,
#                 )
...
#                 )
#                 print(
#                     "[mcp_client] dq_latest_profiling response received",
#                     file=sys.stderr,
#                     flush=True,
#                 )

#                 # Prefer structuredContent["result"] if present
...
# def run_dq_tool(dataset: str):
#     """
#     Call dq_latest_profiling over MCP for the given dataset.

#     You can later generalize this to take tool_name + args.
#     """
#     print("[mcp_client] calling asyncio.run(_runner())", file=sys.stderr, flush=True)
...
#         # Use same python interpreter (venv)
#         server_params = StdioServerParameters(
#             command=sys.executable,
#             args=["-m", "mcp_servers.dq_mcp_server"],
#         )
...
#                 # OPTIONAL: keep this for debug
#                 tools = await session.list_tools()
#                 print(f"[mcp_client] tools detected: {tools}", file=sys.stderr, flush=True)

#                 # 🔹 Actual tool call: dq_latest_profiling
#                 print(
#                     f"[mcp_client] calling dq_latest_profiling for dataset={dataset}",
#                     file=sys.stderr,
#                     flush=True,
...
#                 # Just return the raw response object for now
#                 return response

#     return asyncio.run(_runner())
    
# def run_dq_tool(input_text: str, dataset: str | None = None):
#     print("[mcp_client] calling asyncio.run(_runner())", file=sys.stderr, flush=True)

#     async def _runner():
#         print("[mcp_client] _runner() starting", file=sys.stderr, flush=True)

#         # Use same python interpreter (the venv)
#         server_params = StdioServerParameters(
#             command=sys.executable,
#             args=["-m", "mcp_servers.dq_mcp_server"],
#         )
...
#                 return {"status": "ok", "available_tools": tools}
...
#     # Use the *same* interpreter as the current venv
#     server_params = StdioServerParameters(
#         command=sys.executable,
#         args=["-m", "mcp_servers.dq_mcp_server"],
#         env=None,  # or pass a dict if you need custom env vars
#     )

#     print(
#         f"[mcp_client] launching MCP stdio_client with: {server_params.command} {server_params.args}",
#         file=sys.stderr,
#         flush=True,
#     )
...
#         # Your existing ClientSession logic should look something like this:
#         async with ClientSession(read_stream, write_stream) as session:
#             await session.initialize()
#             print("[mcp_client] session initialized", file=sys.stderr, flush=True)

#             # TEMP: just to verify the round-trip works, you can start with list_tools
#             tools = await session.list_tools()
#             print(f"[mcp_client] tools from server: {tools}", file=sys.stderr, flush=True)

#             # Eventually replace this with your real tool call:
#             # result = await session.call_tool("dq_nl_to_yaml", {"input_text": input_text, "dataset": dataset})
...
#     #     # Example: how you're starting the server – adjust if needed
#     #     server_cmd = ["python", "-m", "mcp_servers.dq_mcp_server"]

#     #     print(f"[mcp_client] launching MCP stdio_client with: {server_cmd}", file=sys.stderr, flush=True)
#     #     async with stdio_client(command=server_cmd) as (read_stream, write_stream):
#     #         print("[mcp_client] connected to MCP server, sending tool call", file=sys.stderr, flush=True)

#     #         # whatever you do to call the tool:
#     #         # e.g. result = await call_tool(...)

#     #         # TEMP: if you have a list_tools or ping tool, call that first
#     #         # result = await client.call_tool("ping", {"text": input_text})

#     #         # Make sure you actually **return** something
#     #         # return result

#     #         # For now, just prove we reached here:
#     #         return {"status": "ok", "msg": "Got into MCP client and out"}
...
# ) -> Any:
#     """
#     Synchronous convenience wrapper for calling a DQ MCP tool.
#     Used by test_mcp_dq.py.
#     """

#     async def _runner():
#         async with DQMCPClient() as client:
