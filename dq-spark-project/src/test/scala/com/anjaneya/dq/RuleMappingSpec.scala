...
class RuleMappingSpec extends AnyFunSuite with BeforeAndAfterAll {
  ...
  test("end-to-end: YAML -> JSON -> ConstraintSpec -> Deequ results for fact_sales") {

    // --- Arrange: Load real specs from JSON generated by Python/Pydantic ---
    // From dq-spark-project/ point of view, config is in ../config
    val jsonPath = "../config/deequ_rules_resolved.json"

    val allSpecs: Seq[ConstraintSpec] =
      ConstraintSpecLoader.loadFromJson(spark, jsonPath)

    val factSpecs = allSpecs.filter(_.table == "fact_sales")
    assert(factSpecs.nonEmpty, "Expected some specs for table fact_sales")

    // We particularly expect:
    //  - a not_null on customer_id (isComplete)
    //  - an in_set on payment_method
    val df = sampleFactSalesDF()

    // --- Act: build a Deequ Check from the real specs and run it ---
    val check: Check =
      DeequRuleMapper.buildCheckForTable("fact_sales", factSpecs)
    ...
    val paymentInSetStatusOpt = constraintResults.find { cr =>
      cr.constraint.toString.contains("ContainedIn") ||
